...
```mermaid
flowchart TD
    classDef entry    fill:#0052CC,stroke:#003E99,color:#FFFFFF,font-weight:bold
    classDef decision fill:#FF991F,stroke:#C27400,color:#000000,font-weight:bold
    classDef reject   fill:#DE350B,stroke:#A52708,color:#FFFFFF,font-weight:bold
    classDef process  fill:#00875A,stroke:#006644,color:#FFFFFF
    classDef store    fill:#403294,stroke:#302570,color:#FFFFFF
    classDef success  fill:#006644,stroke:#004D33,color:#FFFFFF,font-weight:bold
    classDef terminal fill:#7A869A,stroke:#5E6C84,color:#FFFFFF

    subgraph Ingress
        A(["📬 POST /api/waitlist"]):::entry
        B{{"🔒 guardPublicRequest\nrate-limit check"}}:::decision
    end

    subgraph Bot & Abuse Defense
        C(["🚫 429 Too Many Requests"]):::reject
        D["🤖 detectBot\nhoneypot · elapsedMs lt 800ms\nlink regex · disposable domain · repeat-char"]:::process
        E(["🚫 400 opaque\nnever reveals which check fired"]):::reject
        F["⏱️ enforceRateLimit\nSHA-256 IP key + SHA-256 email key\nqueries waitlist_throttle table"]:::process
        G(["🚫 429 opaque message"]):::reject
    end

    subgraph Persistence & Notification
        H["💾 supabaseAdmin INSERT waitlist\nstatus = pending\nconfirmation_token = 64-hex\nexpires_at = now + 7d"]:::store
        I["📧 Send confirmation email\nwith ?token= link"]:::process
    end

    subgraph Double Opt-in Confirmation
        J(["👆 User clicks link"]):::entry
        K["🔑 confirmWithToken\nformat check: /^[a-f0-9]{64}$/"]:::process
        L(["⛔ invalid"]):::reject
        M{{"🔍 Lookup by token\nin waitlist table"}}:::decision
        N(["✅ confirmed\nalreadyConfirmed=true\nidempotent — safe for prefetch"]):::success
        O(["⏰ expired"]):::terminal
        P["✏️ UPDATE waitlist\nstatus = confirmed\nconfirmed_at = now\nconfirmation_token = null"]:::store
        Q(["🎉 Lead confirmed"]):::success
    end
...
flowchart LR
    ...
    subgraph Ingress & Storage
        A(["🌐 Browser CSP violation"]):::entry
        B["📮 POST /api/public/csp-report\nungated — rate-limit only\n60 req/min per IP"]:::process
        C["💾 INSERT csp_reports\nfingerprint = SHA-256(directive + blocked_uri)"]:::store
        D(["🗄️ csp_reports table"]):::store
    end

    subgraph Spike Detection
        E{{"⏰ Scheduled cron\nevery 15 minutes"}}:::entry
        F["🔬 evaluateSpikes\nfetch last 24h — cap 20 000 rows\npartition: recent 15m vs baseline"]:::process
        G{{"📊 Spike detected?\nnew violation (zero baseline)\nOR count ≥ max(10, expected × 4)"}}:::decision
        J{{"🕐 withoutCooldown\nsame fingerprint alerted\nin last 60 min?"}}:::decision
    end

    subgraph Alert Delivery
        I(["😴 No action"]):::terminal
        K["📝 INSERT csp_alerts\nevidence persisted before delivery\nseverity: warning ∥ critical ≥100"]:::reject
        L{{"🔗 CSP_ALERT_WEBHOOK_URL set?"}}:::decision
        M["📋 console.warn ∥ console.error\nworker log only"]:::terminal
        N["🚨 POST webhook\nSlack / Teams / generic sink\nbody: text + severity + fingerprint"]:::success
    end
